...
// Pages Router version with getServerSideProps
const SkillsPagePagesRouter: React.FC<SkillsPageProps> = ({ skills }) => {
  const handleRequestSwap = (skillId: number, skillTitle: string) => {
    alert(`Requesting swap for skill ID: ${skillId}, Title: ${skillTitle}`);
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4 sm:text-5xl">
            Skills Exchange (Pages Router)
          </h1>
          <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto leading-relaxed">
            This version uses Pages Router with getServerSideProps for server-side rendering.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-8 mt-8">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">Available Skills</div>
            </div>
            <div className="text-center">
        ...
        <div className="mb-8">
          <div className="flex flex-wrap justify-center gap-2">
            {['All', 'Frontend', 'Backend', 'Design', 'Data Science', 'Mobile', 'DevOps', 'Marketing', 'Security'].map((category) => (
              <span
                key={category}
                className="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-full text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors"
              >
                {category}
              </span>
            ))}
...
export const getServerSideProps: GetServerSideProps<SkillsPageProps> = async (context) => {
  try {
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    // Dummy skill data - in real app, this would come from an API or database
    const skills: Skill[] = [
      {
        id: 1,
...
export default SkillsPagePagesRouter;